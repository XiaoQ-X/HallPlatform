        ...
        <div class="p-8 space-y-7">
          ...
          <div>
            ...
            <div class="space-y-2">
              <label v-for="(t,i) in current.tasks" :key="i" class="flex items-center gap-3 p-3 rounded-2xl hover:bg-ink-50 cursor-pointer">
                <input type="checkbox" v-model="t.done" class="w-5 h-5 accent-brand-600"/>
          ...
          <div v-if="current.attachments?.length">
            ...
            <div v-for="(a,i) in current.attachments" :key="i" class="flex items-center gap-3 p-3 rounded-2xl bg-ink-50 mb-2">
              <Icon name="folder" class="text-brand-600"/><span class="text-sm text-ink-700">{{ a.n }}</span>
              <span class="ml-auto text-xs text-ink-400">随课程提供</span>
            </div>
          </div>
          ...
          <div class="rounded-3xl border border-ink-100 p-6 bg-ink-50/50">
            ...
            <textarea v-model="content" class="input min-h-32 py-3" placeholder="记录你的探究过程、数据分析与结论…"></textarea>
            ...
            <div v-if="files.length" class="mt-3 space-y-1.5">
              <div v-for="(f,i) in files" :key="i" class="flex items-center gap-2 text-sm bg-white rounded-2xl px-3 py-2">
                ...
                <button class="ml-auto text-ink-400 hover:text-rose-500" @click="files.splice(i,1)"><Icon name="x":size="14"/></button>
              </div>
            </div>
            <button class="btn-primary w-full mt-4" @click="submit">提交课题成果</button>
            <div v-if="current.mine?.grade" class="mt-4 rounded-2xl bg-brand-50 p-4 text-sm">
...
async function open(p){ current.value = await api('/resources/projects/'+p.id);
  ...
  files.value = current.value.mine?current.value.mine.files||[]:[]; }
async function onFile(e){ const f=e.target.files[0];
  const r = await upload(f, p=>upProg.value=p); files.value.push(r); upProg.value=0; }
async function submit(){
  await api('/resources/projects/'+current.value.id+'/submit',
    {method:'POST',body:{content:content.value,files:files.value}});
  current.value = await api('/resources/projects/'+current.value.id);
  alert('提交成功，等待教师评阅');
}
onMounted(async()=>{ rows.value = await api('/resources/projects'); });
</script>
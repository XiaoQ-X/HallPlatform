...
const SIDE_EFFECT_MODEL='side-effects-v1：VH=RH·I·B/d；V0∝I；VE∝I·B；系数需来自实际样品/测量';
...
function sideEffects(input={}){
  ...
  // 厄廷豪森 VE：直接系数 kE(V/(A·T)) 或 温度梯度路径（∇T、S、L_E 三件套），二选一
  const tKeys=['temperatureGradient','seebeckCoeff','ettinghausenLength'];
  const tGiven=tKeys.filter(k=>input[k]!==undefined);
  if(input.ettinghausenCoeff!==undefined&&tGiven.length)throw Error('厄廷豪森系数与温度梯度参数只能提供一种');
  ...
  if(input.ettinghausenCoeff!==undefined){VEamp=seNum(input.ettinghausenCoeff,'厄廷豪森系数 ettinghausenCoeff','V/(A·T)')*I*B;veSource='kE·I·B';}
    ...
    VEamp=Math.abs(S*g*L);veSource='S·∇T·L_E';
  }
  ...
  const Vmeasured=VH+V0+VE+extra;
  ...
  const steps=[
    ...
    '实际测量电压 Vmeasured = VH + V0 + VE + 零点 = '+fmt(Vmeasured)+' V'
  ];
  ...
  return checked({model:SIDE_EFFECT_MODEL,units:{I:'A',B:'T',RH:'m³/C',d:'m'},polarity:{current:sI,field:sB},
    VH,V0,VE,extraVoltage:extra,Vmeasured,
    breakdown:[
...
function correctByReversal(input={}){
  ...
  if(input.readings){
    ...
    readings=quadrants.map(q=>{const se=sideEffects({...input,polarity:{current:q.current,field:q.field}});
      return {name:q.name,current:q.current,field:q.field,VH:se.VH,V0:se.V0,VE:se.VE,extraVoltage:se.extraVoltage,Vmeasured:se.Vmeasured};});
  ...
  const steps=[
    ...
    '修正公式：VH = (V1 − V2 + V3 − V4)/4',
    '代入：('+fmt(V1)+' − ('+fmt(V2)+') + '+fmt(V3)+' − ('+fmt(V4)+'))/4 = '+fmt(correctedHall)+' V',
    '该组合使 V0（∝I）与固定零点偏置正负相消，故被消除',
    'VE 与 VH 同为 I·B 换向规律，四方向对称法无法消除 VE，修正结果实为 VH + VE',
    canSeparate
      ? '已配置厄廷豪森参数，VE(+，+)='+fmt(VEref)+' V，可进一步分离：理想 VH = 修正值 − VE = '+fmt(idealVH)+' V'
      : '未提供厄廷豪森系数/温度梯度，无法分离 VE；如需理想 VH，请补充 ettinghausenCoeff 或温度梯度参数（∇T、S、L_E）'
  ];
  if(notes.length)steps.push('单位提示：'+notes.join('；'));
  return checked({model:SIDE_EFFECT_MODEL,readings,formula:'VH = (V1 - V2 + V3 - V4)/4',
    correctedHall,eliminated:['V0（∝I，随电流换向抵消）','固定零点偏置'],

...
(async()=>{
 ...
 try{
  ...
  await teacher.getByRole('button',{name:'编辑',exact:true}).first().click();
  assert.equal(await teacher.locator('.modal input[placeholder="如 zhangsan"]').count(),0,'匿名展示下编辑表单不得回显真实登录账号');
  assert((await teacher.locator('.modal').innerText()).includes('账号保持不变'));
  ...
  await login(other,'student');await teacher.waitForURL(base+'/');await teacher.getByText('同学1',{exact:true}).first().waitFor();
 ...
 await login(other,'teacher');await teacher.waitForURL(base+'/teacher');await teacher.getByText('教师1',{exact:true}).waitFor();

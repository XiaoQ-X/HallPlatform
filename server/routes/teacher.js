const r=require('express').Router(),bcrypt=require('bcryptjs');const {db,fail,parse,text,number,id,transaction,audit,studentAccess,classAccess}=require('../common');const {forStudent}=require('../grades');
function students(req){return db.prepare("SELECT u.id,u.username,u.name,u.avatar,u.student_no,u.class_id,u.active,c.name class_name FROM users u JOIN classes c ON c.id=u.class_id WHERE u.role='student' AND c.teacher_id=? ORDER BY u.id").all(req.user.id).filter(s=>!req.query.class_id||s.class_id===Number(req.query.class_id));}
...
r.post('/classes',(req,res)=>{const name=text(req.body.name,'班级名称',100);if(db.prepare('SELECT id FROM classes WHERE name=?').get(name))fail('班级已存在');const info=db.prepare('INSERT INTO classes(name,code,teacher_id) VALUES(?,?,?)').run(name,String(req.body.code||'').slice(0,50),req.user.id);res.json({id:info.lastInsertRowid});});
function studentInput(req,key){const b=req.body;const name=text(b.name,'姓名',80);classAccess(req.user,b.class_id);const no=text(b.student_no,'学号',80);if(db.prepare('SELECT id FROM users WHERE class_id=? AND student_no=? AND id<>?').get(b.class_id,no,key||0))fail('班级内学号重复');return{name,no,classId:id(b.class_id),avatar:String(b.avatar||'学生').slice(0,30)};}
function pass(value){text(value,'密码',72);if(value.length<6)fail('密码至少6位');return bcrypt.hashSync(value,12);}
r.post('/students',(req,res)=>{const b=req.body,v=studentInput(req),username=text(b.username,'账号',80),hash=pass(b.password);if(db.prepare('SELECT id FROM users WHERE username=?').get(username))fail('账号已存在');const info=db.prepare("INSERT INTO users(username,password,name,role,class_id,student_no,avatar) VALUES(?,?,?,'student',?,?,?)").run(username,hash,v.name,v.classId,v.no,v.avatar);res.json({id:info.lastInsertRowid});});
r.put('/students/:id',(req,res)=>{const old=studentAccess(req.user,req.params.id),v=studentInput(req,old.id);transaction(()=>{db.prepare("UPDATE users SET name=?,class_id=?,student_no=?,avatar=?,token_version=token_version+1 WHERE id=? AND role='student'").run(v.name,v.classId,v.no,v.avatar,old.id);if(old.class_id!==v.classId)db.prepare('DELETE FROM review_assignments WHERE (reviewer_id=? OR item_id IN (SELECT id FROM review_items WHERE student_id=?)) AND NOT EXISTS(SELECT 1 FROM review_items i JOIN users owner ON owner.id=i.student_id JOIN users reviewer ON reviewer.id=review_assignments.reviewer_id WHERE i.id=review_assignments.item_id AND owner.class_id=reviewer.class_id)').run(old.id,old.id);audit(req.user.id,'student-update',old.id,old,v);});res.json({ok:1});});
